...
const Chatbot: React.FC = () => {
  ...
  return (
    <>
      {/* Tooltip Popup */}
      <div
        className={`fixed bottom-24 right-6 z-50 bg-white px-4 py-3 rounded-xl shadow-xl border border-slate-200 transition-all duration-500 transform origin-bottom-right ${showTooltip && !isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-90 translate-y-4 pointer-events-none'
          }`}
      >
      ...
      <div className={`fixed bottom-6 right-6 z-50 w-[90vw] md:w-[400px] h-[550px] bg-white border border-slate-200 rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300 origin-bottom-right ${isOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0 pointer-events-none'}`}>
        ...
        <div className="bg-primary-700 p-4 flex items-center justify-between shadow-sm">
          ...
          <div className="flex items-center space-x-2">
            <button onClick={() => setIsOpen(false)} aria-label="Close chat assistant" className="text-primary-200 hover:text-white transition-colors p-1">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
